...
def run():
    st.header("📘 28단원. 자기주도 퀀트 전략 생성 시스템")
    st.markdown("""
    “전략을 선택하는 자에서, 전략을 설계하는 자로.”

    - 28.1 맞춤 전략 생성 인터페이스  
      GUI 기반 전략 설계 (RSI, MACD 등 지표 선택, 진입/청산 조건 설정)  
      목표 수익률, 최대 허용 손실 입력  
      자동 조건 보정 기능 (“조건 과도 → 진입 가능성 3% 미만 경고”)

    - 28.2 전략 시각 디버깅 & 복잡도 해석  
      AND/OR 조건 흐름도 자동 생성  
      백테스트 오작동 구간 하이라이트  
      “복잡도↑ = 리스크↑” 매트릭스 자동 분석

    - 28.3 AI 협업 전략 개선 & 학습 루프 지원  
      전략 개선 포인트 자동 제안  
      유사 전략 추천 및 성과 비교  
      Co-Create 모드 (AI 제안 → 사용자 선택 → 재실행)
    """)