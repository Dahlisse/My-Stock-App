...
def run():
    st.header("📘 21단원. 전략 흐름 연결 & 예측 연동 시스템")
    st.markdown("""
    “전략은 흐름이다. 예측과 실행을 연결하라.”

    - 21.1 예측 결과 → 전략 구조 자동 연동  
      예측 확률 기반 전략 가중치 자동 조정  
      실패 확률 상승 시 보수적 전략 자동 전환  
      예: “예측 정확도↓ → 현금 비중↑ + 매매 보류 전략 실행”

    - 21.2 사용자 개입 조정 인터페이스  
      전략 민감도 슬라이더 (보수적 ← 중립 → 공격적)  
      예상 성과 범위(±%) 시각화  
      우선순위 커스터마이징 버튼 (수익 우선 / 리스크 최소화)

    - 21.3 전략 흐름 시각 내비게이션  
      예측 → 전략 판별 → 실행 → 성과 평가 흐름도 시각화  
      전략 변경 히스토리 타임라인 표시  
      내 전략 흐름 요약 해설 생성
    """)