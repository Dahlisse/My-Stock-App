...
def run():
    st.header("📘 22단원. 전략 자동 실행 & 리스크 회피 시스템")
    st.markdown("""
    “수익보다 중요한 건 생존이다. 자동화가 곧 생존전략이다”

    - 22.1 전략 실행 자동화 파이프라인  
      예측 신뢰도 + 매크로 판단 기준 포트 자동 리밸런싱  
      일정 주기 조건 도달 시 자동 전략 교체  
      수동 전환 옵션 병행 (자동 실행 중지 토글)

    - 22.2 리스크 기반 회피 전략 트리거링  
      VIX 급등 / 코스피 급락 / 전술 실패 시 비상 전략 실행  
      회피성 자산 자동 이동 (현금, 금 ETF, 달러 ETF)  
      예상 손실 기반 일시 정지 조건 삽입 (MDD -12% 초과 시 중단)

    - 22.3 자동화 전략 신뢰도 피드백 시스템  
      자동 실행 시 실제 성과 대비 예측 정확도 추적  
      과잉 신뢰/과소 실행 구간 표시  
      예: “예측 실패율 상승 중 → 전략 전환 제안”  
      수동 개입 안내 포함
    """)