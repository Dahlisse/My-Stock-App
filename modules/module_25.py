...
def run():
    st.header("📘 25단원. 시장 심리 사이클 추적 시스템")
    st.markdown("""
    “가격은 감정이 만든다. 시장은 심리의 합성물이다.”

    - 25.1 시장 심리 지표 수집 & 시각화  
      Fear & Greed Index, 공매도 비율, 뉴스 감성, VIX 수집  
      시계열 심리 흐름 추적 (“공포 → 중립 → 탐욕”)  
      “현재 심리 국면: 중립 → 탐욕 이행 중 (61%)” 예측

    - 25.2 전략-심리 적합도 매핑 시스템  
      국면별 전략 매핑 (공포기: 방어적 우위, 탐욕기: 단기 모멘텀 우위)  
      심리-전략 적합도 스코어 시각화 (“전략 A: 공포기 적합도 0.82”)

    - 25.3 심리 과열/침체 경고 시스템  
      뉴스/소셜 감성 분석으로 과열/침체 신호 탐지  
      키워드 급증 감지 (“빚투”, “영끌”, “대박”) → 경고  
      과열/침체 시 전략 자동 보류 또는 리밸런싱 제안
    """)