...
def run():
    st.subheader("📘 2. 재무 실적 분석")
    st.markdown("""
    AI 기반 재무 분석 시스템을 통해 다음 항목을 분석합니다:

    ### 🔹 2.1 핵심 재무제표 분석
    - 매출, 영업이익, 순이익, FCF 등 주요 항목 시계열 시각화
    - PER, PBR, ROE, 유동비율 등 핵심 지표

    ### 🔹 2.2 성장성 및 안정성 평가
    - CAGR, 분기별 성장률, 재무 안정성 종합 점수
    - Altman Z-score 및 배당 지속 가능성

    ### 🔹 2.3 정규화 및 스코어링
    - 업종 대비 percentile 정규화
    - 성장성, 안정성, 수익성, 종합 스코어 제공

    ### 🔹 2.4 사용자 해석 출력
    - 초보자용 간단 설명 + 전문가용 리스크 요인 요약
    - 이익/현금흐름 괴리, 이상징후 자동 감지

    ⚙️ 이 기능은 향후 `module_03`, `module_06`, `module_08`과 통합되어 전략 판단에 활용됩니다.
    """)