...
def compare_strategies(results: list):
    ...
    for col in score_cols:
        if col in df:
            df[col + "_norm"] = scaler.fit_transform(df[[col]]) if col != "최대낙폭" else 1 - scaler.fit_transform(df[[col]])

    df["종합점수"] = df[[c + "_norm" for c in score_cols if c + "_norm" in df]].mean(axis=1)
