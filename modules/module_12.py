...
def run():
    st.header("📘 12단원. 자동 배포 및 CI/CD")
    st.markdown("""
    **“수정 즉시 배포되는 무중단 운영 환경”**

    - GitHub Actions 기반 자동화  
      main/staging 브랜치 분리 → 자동 배포 (Streamlit Cloud 기준)  
      변경 감지 후 자동 푸시/배포, 실패 시 롤백 옵션 포함  

    - API Key & 보안 구성 자동화  
      `.streamlit/secrets.toml` 연동으로 키 관리  
      API 변경 감지 시 자동 갱신 경고 시스템  
      사용자 인증 흐름은 추후 확장 가능성 열어둠  

    - 자동 테스트 연동  
      pytest 기반 핵심 모듈 단위 테스트 자동화  
      푸시 전 테스트 실패 시 자동 취소  
      coverage 모듈 통합 옵션 제공
    """)